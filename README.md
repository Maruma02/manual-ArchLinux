...
Когда мы дали разделу 4G переходим во вкладку "Type" и выбираем Linux Swap. То как у вас должно получиться показанно в скрине сверху.
...
Как вы можете заметить больше места дастается корневому каталогу, у вас должно быть так же.
Если не хотите делать Home, то просто все место отдавайте корневой папке.

После того, как вы разделили диск выбираем "Write". Перед вами вылезет строка в которой обязательно нужно написать "yes" иначе сохранения не сохранятся.
