...
```
Если вы интерисовались как установить себе 2 системы на 1 компьютер, то GRUB будет очень полезен. Здесь так же могут быть решения на ваши проблемы.
[Вот статья про него](https://wiki.archlinux.org/title/GRUB_(Русский)).
